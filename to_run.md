1. `docker-compose up --build -d` from root. or `docker-compose up --build` only. OR for dev mode: `docker-compose -f docker-compose.dev.yml up --build -d`
...
4. To Debug:
   ...
   `docker logs exp4-backend-1`
   `docker logs exp4-worker-1`
